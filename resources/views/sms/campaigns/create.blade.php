<x-app-layout>
    <x-slot name="title">Create SMS Campaign</x-slot>
    <x-slot name="header">
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('sms.campaigns.store') }}" id="campaignForm">
                @csrf
                ...
                <div class="bg-white shadow rounded-lg p-6 mb-4">
                    ...
                    <div class="relative">
                        <textarea id="smsTemplate" name="sms_template" rows="5" required
                            ...
                            placeholder="Hi {consignee_name}, your parcel {waybill_no} worth P{cod_amount} is currently {status}.">{{ old('sms_template') }}</textarea>
                ...
                <div class="bg-white shadow rounded-lg p-6 mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center">
                                <input type="radio" name="recipient_filter_type" value="dynamic" {{ old('recipient_filter_type', 'dynamic') === 'dynamic' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center">
                                <input type="radio" name="recipient_filter_type" value="fixed" {{ old('recipient_filter_type') === 'fixed' ? 'checked' : '' }}
                                    class="text-indigo-600 focus:ring-indigo-500" onchange="toggleFilterType()">
                    ...
                    <div id="dynamicFilters">
                                ...
                                @foreach($statuses as $status)
                                    <label class="flex items-center p-2 border rounded hover:bg-gray-50 cursor-pointer">
                                        <input type="checkbox" name="filter_statuses[]" value="{{ $status->id }}"
                                            {{ in_array($status->id, old('filter_statuses', [])) ? 'checked' : '' }}
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="filter_date_range_days" value="{{ old('filter_date_range_days', 7) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="daily_send_limit" value="{{ old('daily_send_limit') }}"
                        ...
                        <label class="flex items-center mb-4">
                            <input type="checkbox" name="filter_exclude_already_sent" value="1" checked
                                class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                onchange="previewRecipients()">
                    ...
                    <input type="hidden" name="trigger_status_id" id="triggerStatusId">
                </div>
                ...
                <div class="bg-white shadow rounded-lg p-6 mb-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex items-start p-4 border-2 rounded-lg cursor-pointer hover:border-indigo-300 transition" id="methodSim">
                            <input type="radio" name="sending_method" value="sim_based" {{ old('sending_method', 'sim_based') === 'sim_based' ? 'checked' : '' }}
                        ...
                        <label class="relative flex items-start p-4 border-2 rounded-lg cursor-pointer hover:border-indigo-300 transition opacity-60" id="methodGateway">
                            <input type="radio" name="sending_method" value="gateway" {{ old('sending_method') === 'gateway' ? 'checked' : '' }}
                                class="mt-0.5 text-indigo-600 focus:ring-indigo-500" onchange="toggleMethod()">
                            <div class="ml-3">
                    ...
                    <div class="mt-4">
                        ...
                        <input type="number" name="throttle_delay_seconds" value="{{ old('throttle_delay_seconds', 10) }}"
                    ...
                    <div class="mt-4" id="operatorAssignment">
                        ...
                        <select name="assigned_operator_id" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach($smsOperators as $op)
                                <option value="{{ $op->id }}" {{ old('assigned_operator_id') == $op->id ? 'selected' : '' }}>
                                    {{ $op->name }} ({{ $op->email }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="bg-white shadow rounded-lg p-6 mb-4">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="schedule_type" value="immediate" {{ old('schedule_type', 'immediate') === 'immediate' ? 'checked' : '' }}
                            ...
                            <div class="ml-3">
                                ...
                                <span class="block text-xs text-gray-500">Start sending as soon as you click "Start Campaign"</span>
                            </div>
                        </label>
                        <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="schedule_type" value="scheduled" {{ old('schedule_type') === 'scheduled' ? 'checked' : '' }}
                                class="mt-0.5 text-indigo-600 focus:ring-indigo-500" onchange="toggleSchedule()">
                            <div class="ml-3">
                        ...
                        <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="schedule_type" value="recurring_daily" {{ old('schedule_type') === 'recurring_daily' ? 'checked' : '' }}
                                class="mt-0.5 text-indigo-600 focus:ring-indigo-500" onchange="toggleSchedule()">
                            <div class="ml-3">
                        ...
                        <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="schedule_type" value="recurring_hourly" {{ old('schedule_type') === 'recurring_hourly' ? 'checked' : '' }}
                                class="mt-0.5 text-indigo-600 focus:ring-indigo-500" onchange="toggleSchedule()">
                            <div class="ml-3">
                        ...
                        <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="schedule_type" value="custom_cron" {{ old('schedule_type') === 'custom_cron' ? 'checked' : '' }}
                                class="mt-0.5 text-indigo-600 focus:ring-indigo-500" onchange="toggleSchedule()">
                            <div class="ml-3">
                    ...
                    <div id="scheduleFields" class="mt-4 hidden">
                        ...
                        <div id="recurringTimeField" class="hidden">
                            ...
                            <input type="time" name="recurring_time" value="{{ old('recurring_time', '09:00') }}"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div id="recurringHoursField" class="hidden">
                            ...
                            <select name="recurring_interval_hours" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @foreach([1,2,3,4,6,8,12] as $h)
                                    <option value="{{ $h }}" {{ old('recurring_interval_hours') == $h ? 'selected' : '' }}>Every {{ $h }} hour{{ $h > 1 ? 's' : '' }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div id="cronField" class="hidden">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Cron Expression</label>
                            <input type="text" name="cron_expression" value="{{ old('cron_expression') }}"
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium hover:bg-indigo-700 shadow-sm">Create Campaign</button>
                </div>
            </form>
        </div>
    </div>
